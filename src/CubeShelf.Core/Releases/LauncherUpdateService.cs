...
public sealed class LauncherUpdateService
{
    ...
    public LauncherUpdateService(IPlatformPaths paths, HttpClient? httpClient = null, Uri? manifestUri = null)
    {
        ...
        _manifestUri = manifestUri ?? new Uri("https://github.com/zeranemesis/CubeShelf-Launcher/releases/latest/download/release-manifest-v2.json");
        if (!_http.DefaultRequestHeaders.UserAgent.Any()) _http.DefaultRequestHeaders.UserAgent.ParseAdd("CubeShelf/0.8");
    }

    public async Task<LauncherUpdateInfo> CheckAsync(string? currentVersion = null, CancellationToken cancellationToken = default)
    {
        ...
        if (response.Content.Headers.ContentLength > 1024 * 1024) throw new InvalidDataException("Le manifeste de mise à jour est trop volumineux.");
    ...
    public async Task<string> DownloadAsync(LauncherUpdateInfo update, IProgress<double>? progress = null,
        CancellationToken cancellationToken = default)
    {
        ...
        if (artifact.Size is <= 0 or > MaximumArtifactBytes) throw new InvalidDataException("Taille de mise à jour invalide.");
        var directory = Path.Combine(_paths.CacheDirectory, "Updates");
        Directory.CreateDirectory(directory);
        var destination = Path.Combine(directory, $"CubeShelf-{update.LatestVersion}-{CurrentOs()}-{CurrentArchitecture()}.zip");
        ...
        try
        {
            ...
            await using (var output = new FileStream(temporary, FileMode.Create, FileAccess.Write, FileShare.None, 128 * 1024, true))
            {
                ...
                while (true)
                {
                    ...
                    if (read == 0) break;
                    total += read;
                    if (total > artifact.Size) throw new InvalidDataException("La mise à jour dépasse la taille annoncée.");
                    await output.WriteAsync(buffer.AsMemory(0, read), cancellationToken);
                    progress?.Report((double)total / artifact.Size);
                }
                if (total != artifact.Size) throw new InvalidDataException("Téléchargement de mise à jour incomplet.");
            }
            string hash;
    ...
    {
        if (!OperatingSystem.IsWindows()) throw new PlatformNotSupportedException("La mise à jour intégrée est disponible sous Windows uniquement.");
        ...
        foreach (var argument in new[] { Environment.ProcessId.ToString(), Path.GetFullPath(archive), Path.GetFullPath(installDirectory), version })
    ...
    private static string CurrentOs() => OperatingSystem.IsWindows() ? "windows" : OperatingSystem.IsLinux() ? "linux" : "macos";
